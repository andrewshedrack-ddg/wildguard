...
function initInteractiveUI() {
  ...
  if (toggleBtn && sideMenu) {
    toggleBtn.addEventListener('click', () => {
      const hidden = sideMenu.getAttribute('aria-hidden') === 'true';
      sideMenu.setAttribute('aria-hidden', String(!hidden));
      if (hidden) {
        sideMenu.style.display = 'block';
        document.querySelector('main.layout').style.paddingLeft = '260px';
      } else {
        sideMenu.style.display = 'none';
        document.querySelector('main.layout').style.paddingLeft = '24px';
      }
    });
  }
  ...
  accs.forEach((acc) => {
    ...
    head.addEventListener('click', () => {
      ...
      const btn = head.querySelector('.acc-toggle');
      if (btn) btn.textContent = isOpen ? '▾' : '▸';
    });
  });

  // Quick widget toggles (example): mission cards focus
  const cards = Array.from(document.querySelectorAll('.mission-card'));
  cards.forEach((c) => {
    c.addEventListener('click', () => {
      cards.forEach((other) => other.classList.remove('focused'));
